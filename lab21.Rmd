...
**Answer**: Because it demonstrates how many people score from 0-20 range. 

Produce a confidence interval for the mean of your trait.

```{r}
model <- lm_basic(privateness ~ 1, data = pf) 
...
model <- lm_basic(privateness ~ 1, data = hu) 
reg_table(model, level = 0.95)
```
...
**Answer**:Compared to the original dataset, this confidence interval is lower because this less data than the original one. 
...
```{r}
model <- lm_basic(privateness ~ 1 + gender, data = hu) 
reg_table(model, level = 0.95)
```

**Answer**:The difference between male and famale value is 0.1875.
...
**Answer**:No. It challenges traditional gender stereotypes. 

Construct a new dataset that only has ages from 3-49 (from the original `pf`,

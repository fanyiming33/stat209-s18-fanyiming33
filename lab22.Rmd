...
**Answer**: The perfectionism score is the lowest when the speed is fast, and the score is the highest when the speed is low.  Yes, both slopes are significant. 
...
**Answer**:people who have a slower speed are more likely to socre higher scores on perfectionism because they spend more time on their test. 
...
```{r}
model <- lm_basic(privateness ~ 1 + vigilance + reasoning, data = pf ) 
reg_table(model, level = 0.95)
```
...
**Answer**:  There is a stastistical difference for the two traits that I selected. 

Take the previous model and add the variables `speed`, `gender`, and
`country`, the latter lumped into 5 categories.

```{r}
model <- lm_basic(privateness ~ 1 + vigilance + reasoning + speed + gender + fct_lump(country, 5) , data = pf ) 
reg_table(model, level = 0.95)
```

Does this change the slopes for your two traits much?

**Answer**:No, there is no stastistical significance among with 5 countries. 

Summarize the previous model (specifically the relationship between the
three traits) in words.

**Answer**:people with high vigilance scores are likely to be more vigilant. 

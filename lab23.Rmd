...
**Answer**:17 percent crime will result in an arrest
...
**Answer**:about 27 percent of crimes was classfied as a theft.
...
**Answer**:There are 36 thefts for every 1000 houses in Chicago.
...
**Answer**:It's larger. This only shows arrests from area 23 rather than from whole area.
...
**Answer**:larger. That means there are more thefts happening in area 23 than all other area as a whole. 
...
**Answer**:larger. There are more thefts happening in Humboldt park than all other area as a whole. 
...
**Answer**: there is one outlier which has high theft rate with high income level. 

Modify the previous plot so that the points are labeled with the area
name:

```{r}
ggplot(crimes_ca, aes(median_in come, theft_rate)) +
  geom_point() +
  geom_text(label = crimes_ca$area_name)
```
